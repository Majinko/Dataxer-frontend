...
export class ChecklistDatabase {
  ...
  insertItem(parent: CategoryItemNode, id: number, name: string): CategoryItemNode {
    if (!parent.children) {
      parent.children = [];
    }
    const newItem = {id, name} as CategoryItemNode;
    parent.children.push(newItem);
    this.dataChange.next(this.data);
    return newItem;
  }

  insertItemAbove(node: CategoryItemNode, id: number, name: string): CategoryItemNode {
  ...
  insertItemBelow(node: CategoryItemNode, id: number, name: string): CategoryItemNode {
    const parentNode = this.getParentFromNodes(node);
    const newItem = {id, name} as CategoryItemNode;
    if (parentNode != null) {
  ...
  copyPasteItem(from: CategoryItemNode, to: CategoryItemNode): CategoryItemNode {
    const newItem = this.insertItem(to, from.id, from.name);
    if (from.children) {
  ...
  copyPasteItemAbove(from: CategoryItemNode, to: CategoryItemNode): CategoryItemNode {
    const newItem = this.insertItemAbove(to, from.id, from.name);
    if (from.children) {
      from.children.forEach(child => {
        this.copyPasteItem(child, newItem);
      });
  ...
  copyPasteItemBelow(from: CategoryItemNode, to: CategoryItemNode): CategoryItemNode {
    const newItem = this.insertItemBelow(to, from.id, from.name);
    if (from.children) {
      from.children.forEach(child => {
        this.copyPasteItem(child, newItem);
      });
